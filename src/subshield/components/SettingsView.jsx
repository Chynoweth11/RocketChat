...
function BillingSettings({ billing, paymentMethod, invoices, onSaveBilling, onSavePayment }) {
  ...
  const monthlyTotal = useMemo(() => {
    ...
    const seatPrice = Number(billingForm.seatPrice) || 0;
    const base = Number(billingForm.basePrice) || 0;
    return base + seats * seatPrice;
  }, [billingForm.basePrice, billingForm.seatCount, billingForm.seatPrice]);
  ...
  function saveBilling() {
    ...
    onSaveBilling(
      {
        ...billingForm,
        seatCount: Number(billingForm.seatCount),
        seatPrice: Number(billingForm.seatPrice),
      ...
      <section className="ss-card">
        ...
        <div className="ss-field-grid">
          ...
          <SelectField
            ...
            onChange={(value) => setBillingForm((prev) => ({ ...prev, billingCycle: value }))}
          />
        </div>
        <div className="ss-field-grid">
          <Field
            label="Seats"
            value={String(billingForm.seatCount)}
            error={billingErrors.seatCount}
            onChange={(value) => setBillingForm((prev) => ({ ...prev, seatCount: value.replace(/[^\d]/g, "") }))}
          />
          <Field
            label="Seat Price (USD)"
            value={String(billingForm.seatPrice)}
            onChange={(value) => setBillingForm((prev) => ({ ...prev, seatPrice: value.replace(/[^\d.]/g, "") }))}
          />
        </div>
        <div className="ss-field-grid">
          <Field
            label="Base Price (USD)"
            value={String(billingForm.basePrice)}
            onChange={(value) => setBillingForm((prev) => ({ ...prev, basePrice: value.replace(/[^\d.]/g, "") }))}
          />
          <Field
            label="Renewal Date"
            type="date"
            value={billingForm.renewalDate}
            onChange={(value) => setBillingForm((prev) => ({ ...prev, renewalDate: value }))}
          />
        </div>
        <Field
          label="Billing Email"
        ...
        <SettingToggle
          ...
          on={billingForm.autoRenew}
          onToggle={() => setBillingForm((prev) => ({ ...prev, autoRenew: !prev.autoRenew }))}
        />
        <div className="ss-info-grid">
          <Stat label="Estimated monthly total" value={formatMoney(monthlyTotal)} />
          <Stat label="Next renewal" value={formatLongDate(billingForm.renewalDate)} />
        </div>
        <div className="ss-footer">
