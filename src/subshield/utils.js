...
function normalizeRole(raw) {
  ...
  return {
    ...
    permissions: {
      ...
      manageSubscription: Boolean(incoming.manageSubscription ?? incoming.manageBilling),
...
function normalizeSettings(value, fallback = {}, company = {}) {
  ...
  const subscription = source.subscription || source.billing || {};
  ...
  return {
    ...
    emailPreferences: {
      ...
      subscriptionNotices: Boolean(
        emailPreferences.subscriptionNotices ??
          emailPreferences.billingNotices ??
          defaults.emailPreferences?.subscriptionNotices ??
          defaults.emailPreferences?.billingNotices
      ),
    ...
    subscription: {
      ...
      planName: String(
        subscription.planName ?? defaults.subscription?.planName ?? defaults.billing?.planName ?? "Starter"
      ).trim(),
      planCycle: String(
        subscription.planCycle ??
          subscription.billingCycle ??
          defaults.subscription?.planCycle ??
          defaults.billing?.billingCycle ??
          "monthly"
      ).trim(),
      seatCount: Math.max(
        1,
        toNumber(subscription.seatCount ?? defaults.subscription?.seatCount ?? defaults.billing?.seatCount, 1)
      ),
      seatPrice: Math.max(
        0,
        toNumber(subscription.seatPrice ?? defaults.subscription?.seatPrice ?? defaults.billing?.seatPrice, 0)
      ),
      basePrice: Math.max(
        0,
        toNumber(subscription.basePrice ?? defaults.subscription?.basePrice ?? defaults.billing?.basePrice, 0)
      ),
      renewalDate:
        subscription.renewalDate ||
        defaults.subscription?.renewalDate ||
        defaults.billing?.renewalDate ||
        dateFromToday(30),
      autoRenew: Boolean(
        subscription.autoRenew ?? defaults.subscription?.autoRenew ?? defaults.billing?.autoRenew
      ),
      adminEmail: String(
        subscription.adminEmail ??
          subscription.billingEmail ??
          defaults.subscription?.adminEmail ??
          defaults.billing?.billingEmail ??
